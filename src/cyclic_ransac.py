...
class RANSACRegressor(
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        while self.n_trials_ < max_trials:
            ...
            y_pred = base_estimator.predict(X) % 720
        ...
        #rectify inliers
        diff = y_inlier_best[1:] - y_inlier_best[:-1]
        discontinuity = np.argwhere(diff < -500)
        for d in discontinuity:
        	y_inlier_best[int(d)+1:] += 720
        backward_discontinuity = np.argwhere(diff > 500)
        for d in backward_discontinuity:
        	y_inlier_best[int(d)+1:] -= 720
    ...
    def get_coef(self):
        ...
        return self.estimator_.coef_[0], self.estimator_.intercept_
    ...
    def predict(self, X):
        ...
        return self.estimator_.predict(X) % 720
